...
from .vehiclemaeencode import *
from .vehiclemaeencode import VehiclemaeEncode
from .vehiclemaeencode import VTBClassifier
from torch.utils.data import DataLoader

from . import det_utils
from . import boxes as box_ops

def image_attr_loss(image_features, attr_features):

    image_features = F.normalize(image_features, dim=1)
    attr_features = F.normalize(attr_features, dim=1)
    criterion = nn.CosineEmbeddingLoss()
    loss_flag = torch.ones(image_features.shape[0]).cuda()
    loss = criterion(image_features, attr_features,loss_flag)
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.attgru = nn.GRU(768, 256, 1, batch_first=True)
    ...
    def process_proposals(self,images, proposals):

        # 初始化一个空的tensor来存储处理后的proposal
        processed_proposals = []
        ...
        # 提取proposal的向量和坐标
            ...
            for box in vector:
                ...
                # 将proposal调整为（224，224）的图像torch.nn.functionalinterpolate(x, size=(3, 224, 224))
                proposal = torch.nn.functional.interpolate(proposal[None], size=(224, 224), mode='bilinear', align_corners=False)

                # 将处理后的proposal添加到tensor中
                if proposal == None:
                    continue
            ...
            i += 1
            if processed_proposals == None:
                    continue
            proposals_tensor = torch.cat(processed_proposals, dim=0)
        # 将tensor转换为numpy数组并返回
        return proposals_tensor

    def proposals_attr(self,logits,attr_vectors):
        logits_new = []
        #mask_array = np.zeros(logits.shape[0])
        i = 0
        for att_list in logits:
            logits_vectors = []
            #attr_vectors = attr_vectors_old[i]
            att_list_color = att_list[:11]
            color = att_list_color.argmax()
            logits_vectors.append(attr_vectors[color.item()])
                    
            att_list_type = att_list[11:23]
            car_type = att_list_type.argmax()
            logits_vectors.append(attr_vectors[car_type.item()+11])

            att_list_pailiang = att_list[23:27]
            pailiang = att_list_pailiang.argmax()
            logits_vectors.append(attr_vectors[pailiang.item()+23])

            att_list_door = att_list[27:32]
            door = att_list_door.argmax()
            logits_vectors.append(attr_vectors[door.item()+27])

            att_list_seat = att_list[32:42]
            seat = att_list_seat.argmax()
            logits_vectors.append(attr_vectors[seat.item()+32])

            att_list_speed = att_list[42:]
            speed = att_list_speed.argmax()
            logits_vectors.append(attr_vectors[speed.item()+42])

            att_list = torch.stack(logits_vectors)
            #att_list = torch.cat(logits_vectors, dim=1)
            logits_new.append(att_list)
            i += 1 
    ...
    def forward(self,
                ...
                proposals,      # type: List[Tensor]
                images,   # type: List[Tuple[int, int]]
                attr_vectors,
        ...
        box_features = self.box_roi_pool(features, proposals, image_shapes)     #(512*batch_size,256,,7)
            # 将采集样本通过Multi-scale RoIAlign pooling层
            # box_features_shape: [num_proposals, channel, height, width]
            

        processed_proposals = self.process_proposals(images,proposals)
            #将图像分割出每个proposals
            #引入vehicleMAEencode处理RoIAlign输出的特征
            
        new_features,vtb_features = self.VehiclemaeEncode(processed_proposals)       #(512*batch_size,197,768)

        new_features = self.new_features_out(new_features)
        new_features = torch.reshape(new_features,(new_features.shape[0],new_features.shape[1],16,16))
        all_features = torch.cat((box_features,new_features), dim=1)
        ...
        if self.training:
            ...
            logits_vec,hidden_vec = self.attgru(logits_vec)
            hidden_vec = hidden_vec.squeeze(0)
            loss_feature = image_attr_loss(box_features_new,hidden_vec)
        
        all_features = self.box_head(all_features)
        ...
        if self.has_mask():
            ...
            mask_features = self.mask_roi_pool(features, mask_proposals, image_shapes)#torch.Size([18, 256, 14, 14])
            
            mask_features = self.mask_head(mask_features)#torch.Size([23, 256, 14, 14])
