...
def train_one_epoch(model, optimizer, data_loader, device, epoch,attr_vectors,
    ...
    for i, [images, targets] in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            loss_dict = model(images,attr_vectors, targets)
...
@torch.no_grad()
def evaluate(model, data_loader,attr_vectors, device):
    ...
    for image, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        outputs = model(image,attr_vectors)
